...
class DynamicsConnector(BaseConnector):
    ...
    Implements the BaseConnector interface using Dynamics REST API.
    """
    ...
    async def push_bill(self, transaction: Dict[str, Any]) -> Dict[str, Any]:
        print(f"Pushing bill to Dynamics 365: {transaction}")
        return {"platform": "dynamics_365", "status": "success", "platform_id": "dyn_bill_123"}

    async def push_receipt(self, transaction: Dict[str, Any]) -> Dict[str, Any]:
        return {"platform": "dynamics_365", "status": "success"}
